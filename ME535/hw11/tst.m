...
% % Heavy Goland wing (Eastep GolandV5a.nb)
% Lw = 20;    % ft - wing length
% cw = 6;     % ft - wing chord
% bw = cw/2;  % ft - wing half chord - could be variable, see below
% aw = -0.34;  % nondim - wing elastic axis location (0.33c?) aw*bw = distance from 
...
% xthw = 0.43;% nondim - distance from wing elastic axis to wing center of mass.
%             %   (0.43 c ??) NOT USED - included in Sy
% mw = 11.19; % slugs/ft - wing mass per unit length (can be variable)
% Sy = 6.705; % slugs-ft/ft - 
% Iy = 29.145;% slugs-ft^2/ft - Inertia of wing about elastic axis
% E = 1.4976e9; % lb-ft^2 (does he mean lb/ft^2) - modulus
% G = 5.616e8;% lb-ft^2 - (does he mean lb/ft^2)  - shear modulus
% Iea = 1.579e-2;% ft^4 - area moment of inertia of wing in bending
% J = 4.2555e-3; % ft^4 - area moment of inertia of wing in torsion

% Stores (Goland V6b.nb)
% nstores = 3;
% d = 0*bw*[1,1,1]; % 0.2 is x_alpha or how far the CG is behind the elastic axis
%     % See p. 533 of BAH
% mstores = 1*mw*[1,1,1];  % mass of store (assumes 2 ft width)
% Istores = 1*Iy + mstores.*d.^2; % Inertia of store
% Sstores = mstores.*d;     % slugs-ft/ft - Store static imbalance
% astores = 0*[1,1,1];     % -0.2 % ft - store center relative to elastic axis 
% cstores = 10*[1,1,1];    % ft - store chord (see above, length of store in wing chord direction)
% bstores = cstores/2;  % ft - store chord (length)
% ystores = [20*0.975,10,15]./Lw;    % nondim - store position.
% % Doesn't mesh with paper, there the stores have a length along the wing
% swidth = 1; % store width (ft)
% % (span direction).  Where does that come in here?  Through d??

% % Stores (Goland V6b.nb)
% nstores = 1;
% d = 0.2*bw*[1]; % 0.2 is x_alpha or how far the CG is behind the elastic axis
%     % See p. 533 of BAH
% mstores = 1*mw*[1];  % mass of store (assumes 2 ft width)
% Istores = 1*Iy + mstores.*d.^2; % Inertia of store
% Sstores = mstores.*d;     % slugs-ft/ft - Store static imbalance
% astores = -0.3*[1];     % ft - a_s*b_s = store CG Location relative to elastic axis
% cstores = 10*[1];    % ft - store chord (see above, length of store in wing chord direction)
% bstores = cstores/2;  % ft - store chord (length)
% ystores = [20]./Lw;    % nondim - store position.
% % this all assumes that the stores are 1 ft wide - otherwise need to
% % multiply by width of stores!
% % Doesn't mesh with paper, there the stores have a length along the wing
% % (span direction).  Where does that come in here?  Through d??
